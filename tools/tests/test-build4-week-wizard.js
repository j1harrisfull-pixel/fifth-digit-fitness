...
// ---------- 1. Markup: 4 distinct wstep screens, only step 1 visible by default ----------
{
  ...
  ok(/How long, and for how long\?/.test(wizardHtml), 'step 3 asks one plain question');
...
{
  ['bGoal', 'bGoalUnsure', 'bDays', 'scheduleAdv', 'bTime', 'bWeeks', 'bInclude', 'bEquip'].forEach(id => {
    ok(new RegExp('id="' + id + '"').test(SRC), '#' + id + ' still exists (field moved into a wstep, not deleted)');
  });
}

// ---------- 3. Nav: Back/Next buttons exist, Back starts hidden (step 1 has nowhere to go back to) ----------
{
  ok(/id="wizBack" type="button" hidden/.test(SRC), '#wizBack starts hidden on step 1');
...
// ---------- 4. showWeekStep() logic: bounds-clamped, toggles the right things ----------
{
  ...
  ok(/back\.hidden = weekWizardStep <= 1/.test(fn), 'Back is hidden only on step 1');
...
// ---------- 6. openPlan() resets to step 1 every time the sheet opens ----------
{
  const fn = extractFn('openPlan');
  ok(/showWeekStep\(1\)/.test(fn), 'openPlan() always resets the wizard to step 1 (never resumes mid-flow from a stale state)');
}

// ---------- 7. wizNext/wizBack are wired to showWeekStep with the right direction ----------
{
  ok(/\$\("wizNext"\)\.addEventListener\("click", function \(\) \{ showWeekStep\(weekWizardStep \+ 1\); \}\)/.test(SRC), 'Next button advances one step');
  ok(/\$\("wizBack"\)\.addEventListener\("click", function \(\) \{ showWeekStep\(weekWizardStep - 1\); \}\)/.test(SRC), 'Back button retreats one step');
}
