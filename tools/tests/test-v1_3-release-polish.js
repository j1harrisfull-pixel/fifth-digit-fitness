...
{
  // v1.9 Training Flow Trust deploy (approved) bumped the user-facing
  // version string from v1.8 to v1.9 -- the footer format/behavior this test
  // actually guards (a plain user-facing release string, not the internal
  // schema version) is unchanged.
  ok(/Fifth Digit Coach · v1\.9 · works fully offline/.test(SRC),
     'the footer/Settings shows "Fifth Digit Coach · v1.9 · works fully offline" (test 2, v1.9 deploy)');
...
// ---------- v136A/v135/v136B regression guards (unchanged strings must survive) ----------
{
  ...
  ok(/var ctaLabel = started \? "Continue" : "View the session";/.test(SRC),
     'hero CTA reads "View the session" (v1.12.2, approved)');
